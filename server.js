...
db.run(`CREATE TABLE IF NOT EXISTS ml_tokens ( id          INTEGER PRIMARY KEY, access_token  TEXT NOT NULL, refresh_token TEXT NOT NULL, expires_at    INTEGER NOT NULL, user_id       TEXT, created_at    INTEGER DEFAULT (strftime('%s','now')) )`);

// Produtos importados pelo lojista
db.run(`CREATE TABLE IF NOT EXISTS products ( id              TEXT PRIMARY KEY, ml_id           TEXT NOT NULL, title           TEXT NOT NULL, image           TEXT, buy_price       REAL NOT NULL, sell_price      REAL NOT NULL, markup          REAL NOT NULL, link            TEXT, rep_score       REAL, supplier_ml_id  TEXT, active          INTEGER DEFAULT 1, created_at      INTEGER DEFAULT (strftime('%s','now')) )`);

// Clientes
db.run(`CREATE TABLE IF NOT EXISTS customers ( id           TEXT PRIMARY KEY, name         TEXT NOT NULL, email        TEXT, phone        TEXT, address      TEXT, city         TEXT, state        TEXT, zip          TEXT, ml_buyer_id  TEXT, created_at   INTEGER DEFAULT (strftime('%s','now')) )`);

// Pedidos
db.run(`CREATE TABLE IF NOT EXISTS orders ( id              TEXT PRIMARY KEY, customer_id     TEXT NOT NULL, product_id      TEXT NOT NULL, sell_price      REAL NOT NULL, buy_price       REAL NOT NULL, profit          REAL NOT NULL, payment_id      TEXT, payment_status  TEXT DEFAULT 'pending', ml_order_id     TEXT, ml_status       TEXT DEFAULT 'not_placed', ml_sale_id      TEXT, ml_sale_status  TEXT, status          TEXT DEFAULT 'pending', created_at      INTEGER DEFAULT (strftime('%s','now')), updated_at      INTEGER DEFAULT (strftime('%s','now')), FOREIGN KEY(customer_id) REFERENCES customers(id), FOREIGN KEY(product_id)  REFERENCES products(id) )`);

// Pagamentos (Mercado Pago)
db.run(`CREATE TABLE IF NOT EXISTS payments ( id            TEXT PRIMARY KEY, order_id      TEXT NOT NULL, mp_payment_id TEXT, amount        REAL NOT NULL, status        TEXT DEFAULT 'pending', method        TEXT, pix_qr        TEXT, pix_code      TEXT, created_at    INTEGER DEFAULT (strftime('%s','now')), FOREIGN KEY(order_id) REFERENCES orders(id) )`);

// Devoluções
db.run(`CREATE TABLE IF NOT EXISTS returns ( id            TEXT PRIMARY KEY, order_id      TEXT NOT NULL, reason        TEXT NOT NULL, details       TEXT, pix_key       TEXT NOT NULL, status        TEXT DEFAULT 'pending', ml_return_id  TEXT, ml_qr_code    TEXT, refund_id     TEXT, refund_status TEXT DEFAULT 'pending', expires_at    INTEGER, created_at    INTEGER DEFAULT (strftime('%s','now')), updated_at    INTEGER DEFAULT (strftime('%s','now')), FOREIGN KEY(order_id) REFERENCES orders(id) )`);
});
...
const id = uuidv4();
await dbRun(
`INSERT INTO products (id, ml_id, title, image, buy_price, sell_price, markup, link, rep_score) VALUES (?,?,?,?,?,?,?,?,?)`,
...
await dbRun(
`UPDATE products SET sell_price=COALESCE(?,sell_price), markup=COALESCE(?,markup), active=COALESCE(?,active) WHERE id=?`,
...
await dbRun(
`INSERT INTO returns (id, order_id, reason, details, pix_key, ml_return_id, ml_qr_code, expires_at) VALUES (?,?,?,?,?,?,?,?)`,
...
await dbRun(
`UPDATE returns SET status='rejected', details=COALESCE(?||' | Rejeição: '||COALESCE(details,''), details), updated_at=strftime('%s','now') WHERE id=?`,
